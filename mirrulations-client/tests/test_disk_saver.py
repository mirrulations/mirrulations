...
def test_get_extracted_text_attachment_path():
    disksaver = DiskSaver()
    extracted_path = "/EPA/EPA-2003-0001/text-EPA-2003-0001/\
        comments_extracted_text/pdfminer/\
            EPA-2003-0001-0002_attachment_1_extracted.txt"
    expected_attachment_path = "/EPA/EPA-2003-0001/binary-EPA-2003-0001/\
        comments_attachments/"
    actual_path = disksaver.get_extracted_text_attachments_dir(extracted_path)
    assert actual_path == expected_attachment_path
...
def test_update_metadata():
    ...
    with patch('os.listdir') as mocked_attachmentdir:
        test_list_dir = ['1.pdf',
                         '2.pdf',
                         '3.pdf',
                         "1_attachment_1_extracted.txt",
                         "2_attachment_1_extracted.txt",
                         "3_attachment_1_extracted.txt"]
        mocked_attachmentdir.return_value = test_list_dir
        disksaver = DiskSaver()
        test_meta = {
            "DocketID": "EPA-2003-0001",
            "ExtractedPDFSCompleted": len([x for x in test_list_dir
                                           if ".pdf" in x]),
            "TotalPDFs": len([x for x in test_list_dir if ".txt" in x]),
            "Failed/IncompleteExtractions": 0,
            "PercentComplete": 100
        }
        actual_meta = disksaver.\
            generate_metadata_for_extraction(extracted_path)
        assert actual_meta == test_meta
