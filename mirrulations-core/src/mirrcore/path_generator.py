class PathGenerator:
    ...
    following our data structure where every path is based on an
    agencyId, id, docketId and type
    Paths are generated by extracting information from a
    dockets, documents or comments .json file
    Important Keys:
    ----------
    ...
    -------
    get_path(json = dict)
        Looks for the 'type' key in the json and calls the appropriate
        path generator method for the given 'type'
    parse_docket_id(item_id = str)
        If the json does not have the docketId field
        we must parse out the docketId using
        split methods.
    ...
    def get_attributes(self, json_data, is_docket_json=False):
        ...
        agency_id = self._get_nested_keys_in_json(
            json_data, ['data', 'attributes', 'agencyId'],
            None)
    ...
    def get_docket_json_path(self, json):
        agency_id, docket_id, _ = self.get_attributes(json,
                                                      is_docket_json=True)
        base_path = f'/{agency_id}/{docket_id}/text-{docket_id}'
        return f'{base_path}/docket/{docket_id}.json'

    def get_document_json_path(self, json):
        agency_id, docket_id, item_id = self.get_attributes(json)
        base_path = f'/{agency_id}/{docket_id}/text-{docket_id}'
        return f'{base_path}/documents/{item_id}.json'

    def get_document_htm_path(self, json):
        agency_id, docket_id, item_id = self.get_attributes(json)
        base_path = f'/{agency_id}/{docket_id}/text-{docket_id}'
        return f'{base_path}/documents/{item_id}_content.htm'

    def get_comment_json_path(self, json):
        agency_id, docket_id, item_id = self.get_attributes(json)
        base_path = f'/{agency_id}/{docket_id}/text-{docket_id}'
        return f'{base_path}/comments/{item_id}.json'

    def get_attachment_json_paths(self, json):
        '''
        Given a json, this function will return all
        attachment paths for n number attachment links
        '''
        agency_id, docket_id, item_id = self.get_attributes(json)
            ...
            if attributes.get("fileFormats"):
                for file_format in attributes["fileFormats"]:
                    if "fileUrl" in file_format:
                        ...
                        base_path = f'/{agency_id}/{docket_id}'
                        base_path = base_path + f'/binary-{docket_id}'
                        att_path = f'/comments_attachments/{attachment_id}'
                        attachments.append(base_path+att_path)
                    else:
                        print("fileUrl did not exist for attachment")
            else:
                print("fileFormats did not exist for attachment")
